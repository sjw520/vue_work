...
如果父组件给子组件传递数据（函数）：本质起使是子组件给父组件传递数据
...
1. 原生DOM，可以绑定系统事件，click单机事件等等
2. 组件标签，event可以绑定系统事件，native可以把自定义事件变为原生DOM事件

### 2.1 v-mode

结合表单元素一起使用，主要作用收集表单数据
...
//表示父组件字符串传递prop money
//当前子组件绑定了一个自定义事件，而且事件名称为update:money
...
$attrs属于组件的一个属性可以获取到父组件传递过来的props数据

对于子组件而言，父组件给的数据可以利用props接收，如果子组件通过props接收的属性，在$attrs属性当中是获取不到的
...
如果项目在出现很多结构类似功能，想到组件复用

如果项目中很多的组件js业务逻辑相似，想到mixin。可以把多个组件js部分重复，相似的地方